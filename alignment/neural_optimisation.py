...
class NeuralOptimiser:
    """
    Class used to perform optimisation on the alignment of two shapes
    """
    ...
    def optimal_vector_field(self, phi_A, phi_B):
        """
        Compute the optimal deformation field between two shapes
        :param phi_A: The model shape
        :param phi_B: The target shape
    ...
    def similarity_projection2d(self, v, dmu):
        """
        Compute the infinitesimal similarity from a deformation vector field
        :param v: The deformation field
        :param dmu: The measure used for integrating (should be H'(phi))
        :return: THe infinitesimal similarity matrix
        """
        grid = self.grid
        X = grid.X
        Y = grid.Y
    ...
    def step(self, drawers, descent_step=100):
        ...
        :param drawers: Diverse drawers to use
        :param descent_step: The gradient descent step
        :return: The local similarity matrix
        """
    ...
    def optimise(self, nb_iter=100):
        """
        Perform the alignment optimisation between the two shapes
        :param nb_iter:
        :return:
        """
        fig = plt.figure(figsize=(12, 10))
        fig.suptitle('Model and target shapes')
        A_drawer = SdfDrawer(self.grid, fig, 121, 'priori shape')
        B_drawer = SdfDrawer(self.grid, fig, 122, 'target shape')
        fig1 = plt.figure(figsize=(12, 10))
        fig1.suptitle('Global similarity transformation')
